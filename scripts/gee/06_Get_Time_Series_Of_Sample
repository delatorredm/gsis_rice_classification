...
var inName = ee.String('users/ddel528/SampleUnsupClassResult_');
...
var export_TS_extract = maskedS2IC_clip.select('GCVI').map(function(img) {
...
Export.table.toDrive({
  ...
  folder: 'cluster_charts'
});


// Create a time series chart.
var plotGCVI = ui.Chart.image.seriesByRegion({
  imageCollection: maskedS2IC,
  regions: sample.filter(ee.Filter.eq('system:index','00000000000000000000')),
  reducer: ee.Reducer.mean(),
  band: 'GCVI',
  scale: 10,
...
plotGCVI.setChartType('ScatterChart');
plotGCVI.setOptions({
  title: 'GCVI short-term time series',
  hAxis: {title: 'Date'},
  vAxis: {title: 'GCVI'}
});
