 ...
 */
// Center on Object
Map.centerObject(geogBounds,12); // Whole of Iloilo
...
// TEST == Apply to first image in imageCollection
...
Map.addLayer(ee.Image().paint(geogBounds, 0, 2), {palette:'red'}, 'region');
...
// Select one cluster
var oneCluster = result.select("cluster").eq(2).selfMask();

// Visualize single cluster on map
Map.addLayer(oneCluster,{palette:['green']},"only 2", true);
